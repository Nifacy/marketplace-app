...
CREATE OR REPLACE FUNCTION remove_from_favorites(p_customer_id INT, p_product_id INT)
RETURNS INT AS $$
DECLARE
  v_exists INT;
BEGIN
  ...
  SELECT COUNT(*) INTO v_exists FROM favorite_products WHERE customer_id = p_customer_id AND product_id = p_product_id;
  IF v_exists != 0 THEN
    DELETE FROM favorite_products WHERE customer_id = p_customer_id AND product_id = p_product_id;
  END IF;
